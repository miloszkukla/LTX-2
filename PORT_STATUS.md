...
## M7A — blocked at acceptance-gate preflight
...
- Continuation probe: the complete 21-file authenticated one-byte probe was repeated on 2026-08-20 at 05:13:13 UTC from source checkpoint `c7c28d430f2ef343702110a3d6a188c9086b50fd`; the same 17 files succeeded and the same four required files returned HTTP 403. No checkpoint payload was downloaded and the gate remains blocked.
- Access recovery: on 2026-08-20 at 05:25:49 UTC, the complete authenticated one-byte probe succeeded for all 21 pinned files at the recorded revisions. It read 21 bytes total, downloaded no checkpoints, and recorded no token value. Evidence: `artifacts/M7A/prerequisite-recovery.json`, SHA-256 `b495bb84e3918846556ae7be21a7320c1c9487052ee3e6a973408692153a85b3`.
- A100 toolchain preflight: the checked-in bootstrap fallback completed; the Release solution built with 0 warnings and 0 errors; the M1 ABI/native and foundation suites passed all 6 checks with SASS-only `sm_80`; and M2 storage/model-loading passed all 16 checks.
- Earlier-suite blocker: M3 stopped before its C# runner because its accepted Python-oracle manifest is byte-bound to the RTX 5090 device name and contains GPU-dependent last-bit floating-point values. Regeneration on the A100 changes the device to `NVIDIA A100-SXM4-80GB` and several values within the accepted tolerance, so the required byte-for-byte fixture gate fails. M4, M5, and M6 were not run after this required earlier-suite failure.
- Runtime blocker: accepted M6 has no checkpoint-backed C# pipeline runtime. All 12 in-scope `Ltx.Pipelines` classes route through `PipelineExecution` and `SeededMedia`, `PipelineRequest` has no checkpoint/model path, and the trainer exposes only the M6 miniature low-VRAM path. Consequently neither real-checkpoint single-GPU pipeline parity nor the standard-profile trainer smoke test exists to run.
- Acceptance-entry blocker: `scripts/remote/verify-milestone.sh M7A` exits 2 because it accepts only `M0` through `M7` and contains no M7A gate.
- Acceptance impact: implementing the missing real-checkpoint model/pipeline runtime and standard trainer is prerequisite porting work, not standard-profile validation. Calling the retained Python implementation, checking only checkpoint metadata or sampled tensors, continuing with procedural seeded media, or omitting pipelines would silently reduce the approved gate, so no checkpoint payload was downloaded and M7A stopped.
- Scope preserved: no alternate model/revision, Python-backed C# launcher, fixture-only substitute, pipeline omission, or reduced acceptance gate was used. Every accepted M0–M6 plan/status/artifact remains unchanged.
- Redacted evidence: `artifacts/M7A/prerequisite-failure.json`, SHA-256 `c0d0da813cc2e992bd9bfb5070df2b5a339ca58e20531d58f3d83c4421305260`; it contains repository IDs, pinned revisions, paths, status codes, counts, and the continuation recheck but no token, signed URL, credential, or environment value.
- Acceptance evidence: `artifacts/M7A/acceptance-blocker.json`, SHA-256 `cb7ee2d98b41fb83c9a0bd1d89d4588f97c0cdf2084732b12b591b21e8ba8767`, records the commands, exit codes, A100 regression totals, missing runtime surfaces, and required recovery without credentials or environment values.
- Deferred features explicitly remain out of scope: `distilled_mgpu`, `ti2vid_two_stages_mgpu`, and `ti2vid_two_stages_hq_mgpu`; multi-GPU/DDP/FSDP/NCCL/CUDA-IPC/all-to-all including `send_recv_all2all`, `gather_heads`, and `allgather`; B200-only `_fna_kernel` and `_na_kernel`; full fine-tuning; and Hopper-specific native FP8 coverage on this A100.
- Recovery gate: implement checkpoint-backed C# execution for all 12 in-scope single-GPU pipelines, implement the C# standard-profile trainer path, add the exact M7A acceptance entry point, and make every earlier suite reproducible on the A100 without weakening its numerical tolerances; then restart M7A from the complete pinned-file probe.
...
- Known failure: `accepted_m6_missing_m7a_real_checkpoint_runtime`. M7A is not accepted; the real-checkpoint parity and standard-profile trainer gates have not run.
